"""
Hierarchical GRU Model for 6G Bandwidth Allocation
Predicts bandwidth allocation for eMBB, URLLC, and mMTC slices
"""

import torch
import torch.nn as nn


class BandwidthAllocationGRU(nn.Module):
    """
    Hierarchical GRU for 6G Network Slice Bandwidth Allocation
    
    Architecture:
    - Input Layer: 17 features per timestep
    - GRU Layer 1: 64 hidden units (captures immediate temporal patterns)
    - GRU Layer 2: 32 hidden units (captures higher-level trends)
    - Dense Layer: 16 units (non-linear mapping)
    - Output Layer: 3 units (bandwidth for eMBB, URLLC, mMTC)
    
    Args:
        input_size (int): Number of input features per timestep (default: 17)
        hidden_size_1 (int): Hidden units in first GRU layer (default: 64)
        hidden_size_2 (int): Hidden units in second GRU layer (default: 32)
        dense_size (int): Units in dense layer (default: 16)
        output_size (int): Number of output predictions (default: 3)
        dropout (float): Dropout rate for regularization (default: 0.2)
    """
    
    def __init__(self, input_size=17, hidden_size_1=64, hidden_size_2=32, 
                 dense_size=16, output_size=3, dropout=0.2):
        super(BandwidthAllocationGRU, self).__init__()
        
        # Store architecture parameters
        self.input_size = input_size
        self.hidden_size_1 = hidden_size_1
        self.hidden_size_2 = hidden_size_2
        self.dense_size = dense_size
        self.output_size = output_size
        
        # === Layer 1: First GRU (64 units) ===
        self.gru1 = nn.GRU(
            input_size=input_size,
            hidden_size=hidden_size_1,
            num_layers=1,
            batch_first=True,
            dropout=0  # No dropout for single layer
        )
        
        # === Layer 2: Second GRU (32 units) ===
        self.gru2 = nn.GRU(
            input_size=hidden_size_1,
            hidden_size=hidden_size_2,
            num_layers=1,
            batch_first=True,
            dropout=0
        )
        
        # === Dropout for regularization ===
        self.dropout = nn.Dropout(dropout)
        
        # === Layer 3: Dense layer (16 units) ===
        self.fc1 = nn.Linear(hidden_size_2, dense_size)
        self.relu = nn.ReLU()
        
        # === Layer 4: Output layer (3 units) ===
        self.fc_out = nn.Linear(dense_size, output_size)
        
    def forward(self, x):
        """
        Forward pass through the network
        
        Args:
            x: Input tensor of shape (batch_size, seq_len=10, features=17)
        
        Returns:
            Predicted bandwidth allocations (batch_size, 3)
        """
        # === GRU Layer 1 ===
        # Input: (batch, 10, 17)
        # Output: (batch, 10, 64)
        out, _ = self.gru1(x)
        
        # === GRU Layer 2 ===
        # Input: (batch, 10, 64)
        # Output: (batch, 10, 32)
        out, hidden = self.gru2(out)
        
        # === Extract final timestep ===
        # Take only last timestep output
        # Shape: (batch, 32)
        out = out[:, -1, :]
        
        # === Apply dropout ===
        out = self.dropout(out)
        
        # === Dense layer ===
        # Input: (batch, 32)
        # Output: (batch, 16)
        out = self.fc1(out)
        out = self.relu(out)
        out = self.dropout(out)
        
        # === Output layer ===
        # Input: (batch, 16)
        # Output: (batch, 3)
        out = self.fc_out(out)
        
        return out
    
    def get_num_parameters(self):
        """
        Calculate total number of trainable parameters
        
        Returns:
            int: Total number of parameters
        """
        return sum(p.numel() for p in self.parameters() if p.requires_grad)
    
    def print_architecture(self):
        """
        Print detailed model architecture
        """
        print("=" * 70)
        print("MODEL ARCHITECTURE: BandwidthAllocationGRU")
        print("=" * 70)
        print(f"Input size:      {self.input_size} features")
        print(f"GRU Layer 1:     {self.hidden_size_1} hidden units")
        print(f"GRU Layer 2:     {self.hidden_size_2} hidden units")
        print(f"Dense Layer:     {self.dense_size} units")
        print(f"Output size:     {self.output_size} predictions")
        print(f"\nTotal parameters: {self.get_num_parameters():,}")
        print("=" * 70)
